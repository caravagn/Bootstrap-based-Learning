...
# }
	...
	precision = (true_edges+reversed)/(true_edges + reversed +  spurious)
	recall = (true_edges+reversed)/(true_edges + reversed + missing_alarm)
...
# mydata <- apply(x, 2, as.numeric(as.character))


# tprecision = data.frame(row.names = c('Agony', 'Confidence', 'Hill Cl'), stringsAsFactors = FALSE)
# trecall = data.frame(row.names = c('Agony', 'Confidence', 'Hill Cl'), stringsAsFactors = FALSE)
