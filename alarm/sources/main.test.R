...
print(paste('Nboot', nboot, 'p-value', test.pvalue, 'reg ', reg))

# \Pi_boot: perform first bootstrap to estimate the poset
Pi_boot = bootstrap.estimation(data, reg, nboot = nboot)
#head(Pi_boot)
...
Gamma_confidence = bootstrap.estimation_constrained(data, reg, Pi_confidence)
#head(Gamma_confidence)

Gamma_agony = bootstrap.estimation_constrained(data, reg, Pi_agony)
#head(Pi_agony)
Gamma_agony
print('Gamma: OK.')
