...
suppes.bootstrap.inference <- function( dataset, regularization = "bic", suppes.poset, nboot.second = 100, test.pvalue = 0.01, agony_files = paste0(getwd(), "/agony_files"), cores.ratio = 0.9, true_matrix = NA, agony.binaries = './agony', do.hc = TRUE, nboot.first = 100 ) {
    ...
    bootstrap.second.pass[["confidence.poset"]] = bootstrap.estimation.second.pass(dataset = dataset, regularization = regularization, 
        nboot = nboot.second, contraint.poset = suppes.poset)
    ...
    time = as.POSIXct(Sys.time(), format = "%H:%M:%S")
    cat("[*] Estimating the final Bayesian Network with Suppes ...")
    confidence.inference = perform.bn.inference(bootstrap.second.pass[["confidence.poset"]], suppes.poset,test.pvalue,true_matrix)
