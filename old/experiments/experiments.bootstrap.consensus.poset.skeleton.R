...
setwd("~/Desktop/results_bootstrap_consensus_poset_skeleton")

# load the needed scripts and libraries
source("build.consensus.poset.skeleton.R")
...
results_bootstrap_consensus_poset_skeleton = NULL
                ...
                for (samples in names(results_bootstrap[["run"]][[runs]][[var]][["node_size"]][["10"]][["density"]][[den]][["reconstructions"]])) {
                    for (noise_levels in names(results_bootstrap[["run"]][[runs]][[var]][["node_size"]][["10"]][["density"]][[den]][["reconstructions"]][[samples]])) {
                        for (reg in names(results_bootstrap[["run"]][[runs]][[var]][["node_size"]][["10"]][["density"]][[den]][["reconstructions"]][[samples]][[noise_levels]])) {
                            ...
                            curr_consensus_estimation = build.consensus.poset.skeleton(curr_bootstrap_results,curr_dataset,my.curr.reg)
                            curr_stats = getStats(curr_ground_true,curr_consensus_estimation)
                            # save the results
                            results_bootstrap_consensus_poset_skeleton[["run"]][[runs]][[var]][["node_size"]][["10"]][["density"]][[den]][["adj.matrix"]][["adj.matrix.true"]] = curr_ground_true
                            results_bootstrap_consensus_poset_skeleton[["run"]][[runs]][[var]][["node_size"]][["10"]][["density"]][[den]][["reconstructions"]][[samples]][[noise_levels]][[reg]] = curr_consensus_estimation
                            results_bootstrap_consensus_poset_skeleton[["run"]][[runs]][[var]][["node_size"]][["10"]][["density"]][[den]][["stats"]][[samples]][[noise_levels]][[reg]] = curr_stats
                        }
                    }
                }
            #}
...
# save the results
save(results_bootstrap_consensus_poset_skeleton,file="results_bootstrap_consensus_poset_skeleton.RData")
