...
perform.likelihood.fit <- function(dataset, regularization, command = "hc") {
	...
	# I use bnlearn functions...
	if (narcs(my.net) == 0) 
		stop("Empty likelihood fit, aborting .....")
...
bootstrap.estimation.second.pass <- function(dataset, regularization, command = "hc", nboot = 100, random.seed = NULL, verbose = FALSE, 
	contraint.poset) {
...
perform.constrained.likelihood.fit <- function(dataset, poset.adj.matrix, regularization, command = "hc") {
	...
	if (length(nrow(my.arcs)) > 0 && nrow(my.arcs) > 0) {
		for (i in 1:nrow(my.arcs)) {
			...
			adj.matrix[which(colnames(dataset) %in% as.character(my.arcs[i, 1])), which(colnames(dataset) %in% as.character(my.arcs[i, 
				2]))] = 1
		}
	}
